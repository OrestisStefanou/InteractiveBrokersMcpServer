...
| `getQuotes` | Get the latest bid and ask for one or more securities |
| `placeOrder` | Place a market order (write tool) |
| `confirmOrder` | Answer an IB warning to release or abandon an order (write tool) |
| `cancelOrder` | Request cancellation of a working order (write tool) |
...
`getQuotes` (`/iserver/marketdata/snapshot`) returns the bid, ask, sizes, last traded price and session figures for one or more contract IDs.

IB builds the market data subscription on the first request and answers that one with whatever it has, which is frequently nothing. The client detects a response carrying no bid or ask, waits, and asks again, so a caller does not have to know about the warm-up. That means a cold call takes about two seconds longer than a warm one.

Two flags on every quote are worth checking before acting on the numbers:

- `is_stale` is true when IB answered with the previous close instead of a live tick, which it does outside trading hours and when there is no market data subscription. IB signals this by prefixing the value with `C`, which is stripped so the number still parses, and `H` marks a halted security as `is_halted`.
- `is_delayed` is derived from IB's availability code, whose leading letter carries the meaning: `R` realtime, `D` delayed, `Z` delayed and frozen, `Y` frozen, `N` not subscribed. The raw code is kept in `data_availability`.

`spread` is derived by the server as ask minus bid, and is null when either side is missing.
...
`cancelOrder` (`DELETE /iserver/account/{accountId}/order/{orderId}`) requests cancellation of a working order. Acceptance is not cancellation: `submitted: true` means IB took the request, not that the order is dead. A resting order can still fill between the request being accepted and it reaching the exchange, and an order that has already filled cannot be cancelled at all. Call `getOrderStatus` afterwards and confirm the order actually reached `Cancelled` before assuming the position was not taken.

`placeOrder`, `confirmOrder` and `cancelOrder` are the only tools that modify account state, so all three are omitted from the tool list when `READ_ONLY=true`.
